...
def Dice2(output, target, decision, eps=1e-5):
    target = target.float()
    num = 2 * ((output * target).sum(-1).sum(-1) * decision).sum()
    den = output.sum() + target.sum() + eps
...
def softmax_dice3(output, GFLOPs_output, target):
    ...
    loss2 = Dice(output[:, 2, ...], (target == 2).float())
    loss3 = Dice(output[:, 3, ...], (target == 4).float())
    loss_gflops = GFLOPs_output.sum()

    return loss1 + loss2 + loss3 + loss_gflops * 0.02, 1 - loss1.data, 1 - loss2.data, 1 - loss3.data
...
def softmax_dice4(output, decision, GFLOPs_output, target):
    ...
    loss1 = Dice2(output[:, 1, ...], (target == 1).float(), decision)
    loss2 = Dice2(output[:, 2, ...], (target == 2).float(), decision)
    loss3 = Dice2(output[:, 3, ...], (target == 4).float(), decision)
    loss_gflops = GFLOPs_output.sum()

    return loss1 + loss2 + loss3 + loss_gflops * 0.01, 1 - loss1.data, 1 - loss2.data, 1 - loss3.data
